 ...
 * <br>
 * {@link Tidsserierad}ar som blir mottatt av {@link #onEvent(Tidsserierad, long, boolean)} blir lagra til disk via ordinær, ubuffra {@link Writer}-basert blocking I/O.
 * <br>
 * Ved slutten av kvar batch og ved lukking av consumeren, blir eventane flusha til disk.
 *
 */
public class GzipWriterTidsserieradHandler implements TidsserieradHandler {
    private final FileTemplate template;
    public static final String SUFFIX = ".csv.gz";

    private final Map<String, OutputStreamWriter> writers = new HashMap<>();

    public GzipWriterTidsserieradHandler(final FileTemplate template) {
    ...
    private void close(OutputStreamWriter writer, List<IOException> feilVedLukking) {
        try {
            writer.close();
     ...
     * <br>
     * Klienten kan styre kva filer eventen blir skreven til ved å sjå til at kvar event har eit serienummer som i kombinasjon med {@link FileTemplate#createUniqueFile(long, String)}, regulerer output-fila eventen blir ruta til.
     * <br>
     ...
     */
    @Override
    public void onEvent(final Tidsserierad event, final long sequence, final boolean endOfBatch) throws UncheckedIOException, IOException {
        final OutputStreamWriter writer = this.writers.computeIfAbsent(
    ...
    protected OutputStreamWriter newWriter(final Long serienummer, final String filprefix) {
        final File file = template.createUniqueFile(serienummer, filprefix);
        try {
            return gzippedWriter(file, false);
        } catch (IOException e) {
            throw new UncheckedIOException("Klarte ikkje åpne " + file + " for skriving", e);
        }
    }

    public OutputStreamWriter gzippedWriter(final File file, final boolean append) throws IOException {
