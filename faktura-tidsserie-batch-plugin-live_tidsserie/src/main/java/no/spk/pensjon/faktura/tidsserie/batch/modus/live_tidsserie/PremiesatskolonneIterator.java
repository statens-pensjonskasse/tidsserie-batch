 ...
 * <p>
 * <strong>NB!</strong> Denne klassen er ikke trådsikker.
 *
...
class PremiesatskolonneIterator {
    ...
    //Microoptimalisering: Pre-instansiert lambda
    private final Function<Avtale, Optional<Premiesats>> premiesatsMapper;

    private Underlagsperiode currentPeriode;
    private Iterator<String> cache;

    PremiesatskolonneIterator(final Premiesatskolonner premiesatskolonner, final Produkt produkt) {
        this.premiesatsMapper = a -> a.premiesatsFor(produkt);
     ...
     * @throws IllegalStateException dersom underlagsperiode-instansen som det blir hentet premiesatsverdier for endres før alle premiesatskolonner er
     * behandlet
     */
    String nestePremiesatsverdiFor(Underlagsperiode underlagsperiode) {
        if (cache == null) {
            Optional<Premiesats> premiesats = underlagsperiode.valgfriAnnotasjonFor(Avtale.class).flatMap(premiesatsMapper);
